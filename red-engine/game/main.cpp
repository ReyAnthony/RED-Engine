...
void init_map_bindings(RedEngine::Manager* manager)
{
    ...
    RedEngine::MapFactory::bind('6', 0,
				new RedEngine::AnimBinding(false,
							   game_resources::WATER,
							   manager, 4, 50));
...
void init_resources(RedEngine::Manager* game)
{
    ...
    game->addFont(game_resources::SOURCE_CODE_15,
		  "res/fonts/SourceCodePro-Regular.ttf", 15);
...
int main(int argc, char** argv)
{
     ...
     *
     * 	1) EXPLORATION_PHASE
     * 		During this phase, the borg is pathfinding to every 
     *          reachable locations on the map.
     * 		As soon as it cannot find any unvisited place anymore,
     *          the borg will go to the EXIT_CAVE phase.
     *
     * 	2) EXIT_CAVE
     * 		The borg will go to the nearest exit point
     *          (upward if there is no downward exit)
     *
     * 	3) THREAT_ASSESSEMENT
     * 		If there is enemies in the borg's line of sight, 
     *          it will assess the threat.
	...
	new RedEngine::Manager(720, 480, 1280, 720,
			       "RedEngine Test", false, false);
